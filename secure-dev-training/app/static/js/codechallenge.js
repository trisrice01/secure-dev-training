...
async function submitChallengeFlag(challengeId, flagAttempt) {
    ...
    let responseJson;
    try {
        const response = await fetch("/developer/code_editor/challenge/complete", {
            "method": "POST",
...
class FlagProgressBar {
    constructor(totalFlags, iconFactory, totalCompletedFlags, numFlagsPerRow=4) {
        this.stages = []
...
async function getCurrentFlagsProgress() {
    let responseJson;
    try {
        const response = await fetch("/developer/code_editor/challenges", {
            "method": "GET",
            headers: {
                "Content-Type": "application/json"
            }
...
async function getCurrentChallenge() {
    let responseJson;
    try {
        const response = await fetch("/developer/code_editor/current_challenge", {
            "method": "GET",
            headers: {
                "Content-Type": "application/json"
            }
...
function createChallengeElement(challenge, current_challenge) {
    let template = document.getElementById("flag-submission-template-codechallenge");
	...
	clone.querySelector(".current-notification-container").classList.remove("hidden");
    } else {
	clone.querySelector(".locked-notification-container").classList.remove("hidden");
    } 
